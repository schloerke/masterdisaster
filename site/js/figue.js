...
//				if (l.length != len) alert(l.length + "     -     " + len +"     -    " + l);
	...
	function kmeans (k, vectors) {
		...
		while (repeat) {
			...
			repeat = false ;
			for (var j = 0 ; j < k ; j++) {
				if (! compare (newCentroids[j], centroids[j])) {
					repeat = true ; 
					break ; 
				}
			}
...
function compare(thisArr, testArr) {
    if (thisArr.length != testArr.length) return false;
    for (var i = 0; i < testArr.length; i++) {
        if (thisArr[i].length) {                                // TODO !!!! 
            if (!compare(thisArr[i], testArr[i])) return false;
        }
