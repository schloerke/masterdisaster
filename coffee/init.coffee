...
window.play = ->
  if not (status.playing? and status.playing is true)
    reset = false
    if time.get() is timeMax
      reset = true
      time.set(timeMin).notify()
      
    status.interval = setInterval(increment_time, 1000)
    increment_time() if not reset
    status.playing = true
  null
...
$ ->
  ...
  $(".credits a").css("color", "#4C1211")
  $("button.affected").click ->
    $(".credits a").css("color", "#243357")
  $("button.cost").click ->
    $(".credits a").css("color", "#31A354")
